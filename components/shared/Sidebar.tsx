...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setView, userRole, isOpen }) => {
  const navItems = navItemsMap[userRole];

  return (
    <aside className={`w-64 flex-shrink-0 bg-parc-blue-dark text-slate-300 flex flex-col transition-all duration-300 ease-in-out ${isOpen ? 'ml-0' : '-ml-64'}`}>
