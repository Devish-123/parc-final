...
const LeaderboardPodiumItem: React.FC<{
    ...
    const podiumStyles = [
        { // Rank 1
            ...
            icon: <CrownIcon className="w-8 h-8 text-yellow-500 dark:text-yellow-300 absolute top-2 left-1/2 -translate-x-1/2" />
        },
        { // Rank 2
            height: 'h-32',
    ...
    return (
        <div className={`relative flex flex-col items-center justify-end ${style.height} w-1/3 mx-2`}>
            ...
            <div className={`w-full p-2 text-center rounded-t-lg ${style.bgColor} ${style.textColor} ${isCurrentUser ? `ring-4 ring-offset-2 ring-violet-500 dark:ring-offset-slate-800` : ''}`}>
                 <div className={`mx-auto -mt-8 w-14 h-14 rounded-full flex items-center justify-center text-3xl font-bold text-white border-4 ${style.borderColor} bg-slate-700 shadow-lg ${style.shadow}`}>
                    {rank}
                </div>
