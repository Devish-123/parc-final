    ...
    <div>
      <div className="flex justify-between items-center">
        <div>
            ...
            <p className="mt-2 text-violet-600 dark:text-violet-400">Upload and manage training content.</p>
        </div>
      ...
      <div className="mt-8 grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        {materials.map(material => (
          <div key={material.id} className="bg-gradient-to-br from-parc-blue-medium to-parc-blue-dark p-5 rounded-xl shadow-lg border-slate-700/50 flex flex-col justify-between transition-all duration-300 transform hover:shadow-xl hover:-translate-y-1">
            <div>
              <div className="flex items-center gap-3">
                <div className="p-2 bg-white/10 rounded-full">
                  <BookOpenIcon className="w-5 h-5 text-white" />
                </div>
                <span className="text-xs font-semibold bg-white/10 text-slate-200 px-2 py-1 rounded-full">{material.type}</span>
              </div>
              <h3 className="mt-4 text-lg font-bold text-white">{material.title}</h3>
              <p className="text-sm text-slate-300">{material.course}</p>
              <p className="mt-2 text-sm text-slate-300 line-clamp-2">{material.content}</p>
            </div>
            <div className="mt-4 flex items-center gap-2">
                 <button onClick={() => handleViewMaterial(material)} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-center text-white bg-white/10 rounded-lg hover:bg-white/20">
                    <EyeIcon className="w-4 h-4" />
                    View
                </button>
                 <button onClick={() => handleGenerate(material, AssessmentType.TEST)} disabled={material.type === MaterialType.VIDEO} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-center text-white bg-sky-500/50 rounded-lg hover:bg-sky-500/80 disabled:bg-slate-600/50 disabled:cursor-not-allowed">
                     <GenerateIcon className="w-4 h-4" />
                     Test
                 </button>
                 <button onClick={() => handleGenerate(material, AssessmentType.ASSIGNMENT)} disabled={material.type === MaterialType.VIDEO} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-center text-white bg-amber-500/50 rounded-lg hover:bg-amber-500/80 disabled:bg-slate-600/50 disabled:cursor-not-allowed">
                     <GenerateIcon className="w-4 h-4" />
                     Assignment
                 </button>
            </div>
